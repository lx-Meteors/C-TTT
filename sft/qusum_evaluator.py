...
from qusum_prepare_data import get_examples
...
def parse_args():
    ...
    parser.add_argument('--work_dir', type=str, default='../experiment/full_train_full_test_15x/sac_pwc_vanilla', required=False,
...
class Evaluator:
    ...
    def draw_ema_loss(self, alpha):
        ...
        lm_loss_values = [-1 if 'lm_loss' not in entry['training_loss'] else entry['training_loss']["lm_loss"] for entry
                          in info_list]
    ...
    def evaluate(self, rank=0):
        ...
        print(
            f"[INFO] GPU{rank}: eval_examples[{rank * example_num_per_gpu}:{rank * example_num_per_gpu + len(eval_examples)}], nums:{len(eval_examples)}")
